...
public class Clock {
	...
	public void update() {
		...
		float delta = calcDeltaTime(currUpdate);

		if(!isPaused) {
			updateElapsedCycles(delta);
			updateExcessCycles(delta);
		}
		
		this.lastUpdate = currUpdate;
	}

	private void updateExcessCycles(float delta) {
		this.excessCycles = delta % millisPerCycle;
	}

	private void updateElapsedCycles(float delta) {
		this.elapsedCycles += (int)Math.floor(delta / millisPerCycle);
	}

	private float calcDeltaTime(long currUpdate) {
		return (float) (currUpdate - lastUpdate) + excessCycles;
	}
	...
	/**
	 * We must not be careless in exposing private member variables to change.
	 * @param cycles
	 */
	public void setElapsedCycles(int cycles){
