...
public class SnakeGame extends JFrame {
	...
	private void resetGameIfNotInProgress() {
	...
	private void addNorthToDirectionsList() {
		if(isGamePlaying()) {
			if(isDirectionListNotFull()) {
				Direction last = directions.peekLast();
				if(isMostRecentDirectionAdjacentToNorth(last)) {
	...
	private void addEastToDirectionsList() {
		if(isGamePlaying()) {
			if(isDirectionListNotFull()) {
				Direction last = directions.peekLast();
	...
	private void addWestToDirectionsList() {
		if(isGamePlaying()) {
			if(isDirectionListNotFull()) {
				Direction last = directions.peekLast();
				if(isMostRecentDirectionAdjacentToWest(last)) {
	...
	private void addSouthToDirectionsList() {
		if(isGamePlaying()) {
			if(isDirectionListNotFull()) {
				Direction last = directions.peekLast();
				if(isMostRecentDirectionAdjacentToSouth(last)) {
	...
	private boolean isMostRecentDirectionAdjacentToWest(Direction last) {
		return last != Direction.East && last != Direction.West;
	}

	private boolean isMostRecentDirectionAdjacentToSouth(Direction last) {
		return last != Direction.North && last != Direction.South;
	}

	private boolean isMostRecentDirectionAdjacentToNorth(Direction last) {
		return last != Direction.South && last != Direction.North;
	}

	private boolean isDirectionListNotFull() {
		return directions.size() < MAX_DIRECTIONS;
	}
...
//		initializeEverything();
	...
	private void initializeEverything() {
		...
		this.logicTimer = new Clock(CYCLES_PER_SECOND);
		this.isNewGame = true;
	}
	...
	private void updateGame() {
		...
		detectCollisionType(collision);
	}

	private void detectCollisionType(TileType collision) {
		/*
